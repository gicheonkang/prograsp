...
def eval_caption(task, generator, models, sample, **kwargs):
    ...
    for i, sample_id in enumerate(sample["id"].tolist()):
        ...
        results.append({"image_id": str(sample_id), "result": detok_hypo_str.translate(transtab).strip()})
...
def eval_refcoco(task, generator, models, sample, **kwargs):
    ...
    results = [
        {"uniq_id": sample_id, "result": hyps[i].tolist()}
        for i, sample_id in enumerate(sample["id"].tolist())
    ]
    
    score = None
    if 'score' in gen_out[0][0].keys():
