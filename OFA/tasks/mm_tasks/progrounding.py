...
from data.mm_data.progrounding_dataset import progroundingDataset
...
@dataclass
class progroundingConfig(OFAConfig):
    eval_acc: bool = field(
...
@register_task("progrounding", dataclass=progroundingConfig)
class progroundingTask(OFATask):
    def __init__(self, cfg: progroundingConfig, src_dict, tgt_dict):
    ...
    def load_dataset(self, split, epoch=1, combine=False, **kwargs):
        ...
        self.datasets[split] = progroundingDataset(
            split,
            dataset,
            self.bpe,
            self.src_dict,
    ...
    def valid_step(self, sample, model, criterion):
        ...
        if self.cfg.eval_acc:
            ...
            hyps_box_list = []
            refs_box_list = []
            hyps_box_num = int(hyps.size()[1]/4)
            refs_box_num = int(refs.size()[1]/4)
            for i in range(hyps_box_num):
                box = hyps[:, 4*i:4*i+4]
                hyps_box_list.append(box)
            for i in range(refs_box_num):
                box = refs[:, 4*i:4*i+4]
                refs_box_list.append(box)
            score_sum = 0
            for i, box_hyps in enumerate(hyps_box_list):
                for j, box_refs in enumerate(refs_box_list):
                    scores = self._calculate_ap_score(box_hyps, box_refs)
                    score_sum += scores.sum().item()
            logging_output["_score_sum"] = score_sum #scores.sum().item()
            logging_output["_score_cnt"] = refs_box_num #scores.size(0)
