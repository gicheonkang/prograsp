...
class gcDataset(OFADataset):
    def __init__(
        ...
        if type(bpe).__name__ == 'GPT2BPE':
            self.prompt = ' what does the region describe? region: '
        else:
    ...
    def __getitem__(self, index):
        ...
        #tgt_caption = self.pre_caption(text, self.max_src_length)
        text = text.translate(self.transtab).strip()
        caption_token_list = text.strip().split()
        tgt_caption = ' '.join(caption_token_list[:self.max_tgt_length])

        src_item = self.encode_text(self.prompt) #.format(region_coord)) #, use_bpe=False)
