...
class questionerDataset(OFADataset):
    def __init__(
        self,
        split,
        dataset,
        ...
        if type(bpe).__name__ == 'GPT2BPE':
            self.prompt = ' given the dialogue history " {} ", what can I ask about the region? '
        else:
            print("wrong BPE!!!!!!!!!!!!!!!!!!!!!!!! Please use GPT2BPE ........")
            exit()
    ...
    def __getitem__(self, index):
        ...
        uniq_id, base64_str, question, region_coord, dialogue = self.dataset[index]
        ...
        caption_token_list = question.strip().split()
        tgt_question = ' '.join(caption_token_list[:self.max_tgt_length])
        ...
        src_item = self.encode_text(self.prompt.format(source))
        tgt_item = self.encode_text(" {}".format(tgt_question))
        coord_item = self.encode_text(region_coord, use_bpe=False)

        src_item = torch.cat([self.bos_item, src_item, coord_item, self.eos_item])
