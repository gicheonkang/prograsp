...
class answererDataset(OFADataset):
    def __init__(
        self,
        split,
        dataset,
        ...
        if type(bpe).__name__ == 'GPT2BPE':
            self.prompt = ' does the question " {}? " describe the region? region: '
            #self.prompt = ' {}? region: '
        else:
    ...
    def __getitem__(self, index):
        ...
        uniq_id, base64_str, answer, region_coord, question = self.dataset[index]
        ...
        answer = answer.translate(self.transtab).strip()
        caption_token_list = answer.strip().split()
        tgt_answer = ' '.join(caption_token_list[:self.max_tgt_length])

        src_item = self.encode_text(self.prompt.format(question)) #.format(region_coord)) #, use_bpe=False)
        tgt_item = self.encode_text(" {}".format(tgt_answer))
